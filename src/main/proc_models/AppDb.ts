...
class AppDb {
  ...
  async search_file(): Promise<DataTypes.Resp<DataTypes.SearchFileResp>> {
    ...
    try {
      if (!this.db) throw new Error('Database not initialized')
      const fileModels = await this.db.all<DataTypes.FileModel[]>('SELECT * FROM files')
      ...
      for (const fileModel of fileModels) {
        ...
        const fileInfo: DataTypes.File = {
          id: fileModel.id == undefined ? 0 : fileModel.id,
          name: fileModel.name,
          path: fileModel.path,
          startTimeSec: fileModel.startTimeSec,
          endTimeSec: fileModel.endTimeSec,
  ...
  async getVideoById(
    ...
    try {
      if (!this.db) throw new Error('Database not initialized')
      const videoData = await this.db.get<DataTypes.FileModel>('SELECT * FROM files WHERE id = ?', [
        video.id
      ])
      if (videoData) {
