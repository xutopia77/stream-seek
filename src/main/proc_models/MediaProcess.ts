...
async function make_split_info(req: Request): Promise<SplitInfo[] | Response> {
  const processSplitInKeyFrame = (splitInfo: SplitInfo[], keyFrameSplitInfo: FrameInfo[]): SplitInfo[] => {
...
async function cutVideo(req: Request): Promise<Response> {
  let resp: Response = { code: 0, status: 'success' };
  const filepath = req.data.filepath;

  function makeDistFileName(filepath: string, startTimeIn: number, endTimeIn: number): string | null {
    ...
    let startTimeSec = startTimeIn + baseStartTimeSec;
    let endTimeSec = endTimeIn + baseStartTimeSec;
    logger.log(`filename ${filename}, startTime ${startTime}, baseStartTimeSec ${baseStartTimeSec}`);
    logger.log(`cut param: start ${startTimeIn}, end ${endTimeIn}, calc start ${startTimeSec}, end ${endTimeSec}`);
