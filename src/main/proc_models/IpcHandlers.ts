...
async function handle_open_folder(mainWindow: Electron.BrowserWindow, req: Request): Promise<Response> {
  ...
  if (!canceled) {
    ...
    let resp: Response = { code: 0, status: 'success', bOver: false, data: { folder: folderPath } };
...
async function handle_save_prj(req: Request): Promise<Response> {
  // 把req.data 保存到文件
  let filePath = make_file_prj_path(req.data.filename);
...
function handle_app_start(): Response {
  let resp: Response = { code: 0, status: 'success', bOver: true, data: {} };
  // 读取cfg.json

  let cfgPath = path.join(appCfg.appData, 'prj.json');
  let data = '';
  try {
...
export class IpcHandlers {
  ...
  handle_event = async (event: string, ...args: string[]): Promise<Response> => {
    ...
    let resp = await this.start_process_cmd(req);
