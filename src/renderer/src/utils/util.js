...
function updateKeyframeSplitInfo(frameInfo) {
  ...
  for (let i = 0; i < frameInfo.length; i++) {
    ...
    if (frame.pict_type == 'I') {
      const curTime = frame.pts_time - appStore.videoPlayCtrl.videoStartTime
      if (curTime > lastTime) {
  ...
  if (lastTime != 0.0) {
    let itemInfo = util.makeSplitInfo()
    itemInfo.startTime = lastTime
    itemInfo.endTime = appStore.curVideoInfo?.mediaInfo.duration
    itemInfo.color = 'yellow'
    frameSplitInfo.push(itemInfo)
  }
  util.splitInfoCorrect(frameSplitInfo, appStore.curVideoInfo?.mediaInfo.duration)
  return frameSplitInfo
}

async function getKeyFrameInfo(ipcAPi) {
  let resp = { code: 0, status: 'success' }
...
*/
function clear_cur_slt_video_info(req) {
  function clear_videoPlayCtrl() {
  ...
  if (req != null) {
    if (req.clearModel == 'changeToThumbnail') {
      ...
      clear_videoPlayCtrl()
      if (appStore.curVideoInfo?.mediaInfo?.duration !== undefined) {
        appStore.curVideoInfo.mediaInfo.duration = tmpDuration
      }
...
function folder_file_proc(fileinfo) {
...
function process_work_response(workRespose) {
...
function processVideoEvent(events) {
  ...
  // // 这里先把eventInTimePoint的event的distance做一个滤波处理，避免跳动幅度过大，这里取5个点的平均值
  // for (let i = 0; i < eventInTimePoint.length; i++) {
  //   let e = eventInTimePoint[i]
  //   let sum = 0
  //   let count = 0
  //   for (let j = i - 2; j <= i + 2; j++) {
  //     if (j < 0 || j >= eventInTimePoint.length) {
  //       continue
  //     }
  //     sum += eventInTimePoint[j].distance
  //     count++
  //   }
  //   e.distance = sum / count
  // }

  // console.log('eventInTimePoint:', eventInTimePoint)
  ...
  // console.log('生成的颜色分段数据:', colorSegments)
  appStore.barColorCfg = colorSegments
}

function processSplitInfo() {
...
async function get_slt_video(ipcAPi, video) {
  let response = await ipcAPi.trigger_event(
    JSON.stringify({ cmd: 'slt_video', data: { src: video.filePath } })
  )
  if (response.code != 0) {
    console.log('slect video failed')
  } else {
    // console.log('slect video success', response)
...
// 把秒数字转换成为年月日时分秒
const formatSecond2Time = (timeSec) => {
...
const save_project = async (ipcAPi) => {
...
function process_heartbeat(response) {
  ...
  const appStatus = response.data ? response.data.appStatus : ''
...
class Util {
  ...
  getFilenameFromPath(filePath) {
    ...
    // const filePath = 'D:\\02_workspace\\05_timeCapsule\\01_stream_manager_ui\\stream_manager_ui\\src\\data\\00_20250310042708_20250310051906.mp4';
  ...
  splitInfoCorrect(splitInfos, videoDuration) {
